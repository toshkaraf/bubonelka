...
class ThemeClass {
  ...
  final String? nextRepetitionDate;
  final int parentId;
  final List<String> levels;
  final List<String> imagePaths;
  final int position;
  final double easeFactor;
  final int isMastered; // ✅ ДОБАВЛЕНО
  ...
  ThemeClass({
    ...
    this.position = 0,
    this.easeFactor = 2.5,
    this.isMastered = 0, // ✅ по умолчанию 0
  });
  ...
  factory ThemeClass.fromMap(Map<String, dynamic> map) {
    return ThemeClass(
      ...
      nextRepetitionDate: map['next_repetition_date'],
      parentId: map['parent_id'] ?? 0,
      levels: (map['level'] as String? ?? '')
          .split(';')
      ...
      isMastered: map['is_mastered'] ?? 0, // ✅ ДОБАВЛЕНО
    );
  }
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'next_repetition_date': nextRepetitionDate,
      'parent_id': parentId,
      'level': levels.join(';'),
      'image_paths': imagePaths.join(';'),
      'position': position,
      'ease_factor': easeFactor,
      'is_mastered': isMastered, // ✅ ДОБАВЛЕНО
    };
  }
  ...
  ThemeClass copyWith({
    ...
    String? nextRepetitionDate,
    int? parentId,
    List<String>? levels,
    List<String>? imagePaths,
    int? position,
    double? easeFactor,
    int? isMastered, // ✅ ДОБАВЛЕНО
  }) {
    return ThemeClass(
      id: id ?? this.id,
      ...
      nextRepetitionDate: nextRepetitionDate ?? this.nextRepetitionDate,
      parentId: parentId ?? this.parentId,
      levels: levels ?? this.levels,
      imagePaths: imagePaths ?? this.imagePaths,
      position: position ?? this.position,
      easeFactor: easeFactor ?? this.easeFactor,
      isMastered: isMastered ?? this.isMastered, // ✅ ДОБАВЛЕНО
    );
  }
}
...
extension ThemeStage on ThemeClass {
  int get currentStage {
    if (timeOfLastRepetition == null || nextRepetitionDate == null) {
      return 0; // Новая тема или нет данных
    }
    ...
    if (lastDate == null || nextDate == null) {
      debugPrint('⚠️ Ошибка парсинга дат у темы: $themeNameTranslation');
      return 0;
    }

    final diffMinutes = nextDate.difference(lastDate).inMinutes;

    if (diffMinutes <= 0) {
      debugPrint(
          '⚠️ Некорректный интервал у темы: $themeNameTranslation (diffMinutes=$diffMinutes)');
      return 0;
    }
  ...
  bool get isMastered => currentStage == 6;
}
